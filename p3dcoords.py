    ...
    def __getMousePos(self, task):
        ...
        for pixel in pixels:
            if sizing[0] > sizing[1]:
                mpos = LPoint2f(((pixel[0]-sizing[0])/sizing[1]), (sizing[1]-pixel[1])/sizing[1])
            else:
                mpos = LPoint2f(((pixel[0]-sizing[0])/sizing[0]), (sizing[1]-pixel[1])/sizing[0])
            pos3d = Point3()
            nearPoint = Point3()
            farPoint = Point3()
...
if 'addxtra' in coordls and 'isize' in coordls['addxtra']:
    sizing = [coordls['addxtra']['isize'][0]/2, coordls['addxtra']['isize'][1]/2]
pvector = [bcenter[0]-campos[0], bcenter[1]-campos[1], bcenter[2]-campos[2]]
pixels = coordls['pixel']
print (f"Sizing is {sizing}")
YourClass()
base.taskMgr.run()